...
export function Discover() {
    ...
    return (
        <div className="w-full h-[calc(100vh-6rem)] md:h-[calc(100vh-4.5rem)] bg-bg overflow-hidden flex items-center justify-center">
            <div className="flex flex-col md:flex-row gap-4 h-full w-full overflow-hidden p-2 md:p-4">

                {/* 左侧：探索控制面板 (桌面固定，移动变为可滚动吸顶) */}
                <div className="w-full md:w-64 shrink-0 h-auto md:h-full bg-card/50 backdrop-blur-sm border border-line/60 rounded-2xl p-4 md:p-5 flex flex-col gap-4 md:gap-6 overflow-hidden">
                    <div className="flex justify-between items-center border-b border-line/30 pb-2 shrink-0">
                    ...
                    {/* 过滤器 (手机上直接用原生滚动，内容不多不影响) */}
                    <div className="flex-1 overflow-y-auto custom-scrollbar pr-1 md:pr-2 pb-2">
                        <DiscoverFilters
                    ...
                    <div className="mt-auto pt-4 border-t border-line/30 flex flex-col items-center gap-2 shrink-0">
                        <button
                            ...
                            className="w-full py-2 bg-primary/10 border border-primary/30 rounded-xl text-primary text-[11px] font-mono font-bold hover:bg-primary/20 transition-all flex items-center justify-center gap-1.5 cursor-pointer"
                        >
                            🎲 Shuffle the Shelf
                        </button>
                ...
                <div className="flex-1 flex flex-col bg-card/30 border border-line/50 rounded-2xl overflow-hidden relative h-full">
                    {/* 顶部页眉 */}
                    <div className="px-4 md:px-6 py-2 border-b border-line/30 flex justify-between items-center bg-bg/50 shrink-0 min-h-[48px]">
                        <h1 className="font-display font-bold text-base md:text-lg text-ink hero-title">
                            Discover
                        </h1>
                    ...
                    <div className="flex-1 overflow-hidden p-4 md:p-6 relative">
                        {loading ? (
                            ...
                            <div className="flex flex-col items-center justify-center h-full text-center space-y-2 bg-bg2/50 rounded-2xl border border-line/40 p-6 md:p-10">
                    ...
                    {!loading && processedBooks.length > 0 && (
                        <div className="shrink-0 px-3 md:px-6 py-2 md:py-3 border-t border-line/20 bg-bg/40 flex items-center justify-between gap-2">
                            <button
                                disabled={currentPage === 1}
                                onClick={() => setCurrentPage(p => Math.max(1, p - 1))}
                                className="px-3 md:px-5 py-1.5 md:py-2 rounded-lg border border-line/60 bg-bg2 text-[11px] md:text-xs font-mono font-medium text-ink hover:border-tertiary disabled:opacity-30 disabled:pointer-events-none transition-all cursor-pointer whitespace-nowrap"
                            >
